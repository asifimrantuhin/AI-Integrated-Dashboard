...
const InventoryForecastChart = ({ startDate, endDate, companyId, days = 30, forecastData, historicalData }) => {
  ...
  useEffect(() => {
    ...
    const fetchForecast = async () => {
      try {
        ...
        if (!historicalData) {
          const histResponse = await api.get('/inventory/trends', {
            params: { company_id: companyId || 1, months: 12 },
          });
          setHistorical(histResponse.data || []);
        }

        if (!forecastData) {
          const response = await api.post('/ai/forecast/inventory', {
        ...
        setError(err.response?.data?.error || 'Failed to load inventory forecast');
      } finally {
        setLoading(false);
      }
    };
  ...
  const historicalValues = (historical || []).map((item) => item.amount ?? 0);
    ...
    <Card>
      <CardContent>
        <Box display="flex" justifyContent="space-between" alignItems="center" mb={2}>
          <Typography variant="h6">Inventory Forecast</Typography>
          <Chip
