...
const FinanceForecastChart = ({ startDate, endDate, days = 30, forecastData, historicalData }) => {
  ...
  useEffect(() => {
      ...
      try {
        ...
        if (!historicalData) {
          const histResponse = await api.get('/finance/budget-vs-actual', {
            params: { yearMonth: startDate?.slice(0, 7) },
          });
          setHistorical(histResponse.data || []);
        }

        if (!forecastData) {
          const response = await api.post('/ai/forecast/finance', {
        ...
        setError(err.response?.data?.error || 'Failed to load financial forecast');
      } finally {
        setLoading(false);
      }
    };
    ...
    fetchForecast();
  }, [forecastData, historicalData, startDate, endDate, days]);
  ...
  const historicalValues = (historical || []).map((item) => item.actual ?? item.actual_amount ?? 0);
...
export default FinanceForecastChart;
