...
const SupplyForecastChart = ({ startDate, endDate, companyId, days = 30, forecastData, historicalData }) => {
  ...
  useEffect(() => {
    ...
    const fetchForecast = async () => {
      try {
        ...
        if (!historicalData) {
          const histResponse = await api.get('/supplychain/overview', {
            params: { yearMonth: startDate?.slice(0, 7), company_id: companyId },
          });
          setHistorical(histResponse.data?.trend || []);
        }

        if (!forecastData) {
          const response = await api.post('/ai/forecast/supplychain', {
            forecast_type: 'supplychain',
        ...
        setError(err.response?.data?.error || 'Failed to load supply chain forecast');
      } finally {
        setLoading(false);
      }
    };
    ...
    fetchForecast();
  }, [forecastData, historicalData, startDate, endDate, companyId, days]);
  ...
  const historicalValues = (historical || []).map((item) => item.po_value ?? item.poValue ?? 0);
    ...
    <Card>
      <CardContent>
        <Box display="flex" justifyContent="space-between" alignItems="center" mb={2}>
          <Typography variant="h6">Supply Chain Forecast</Typography>
          <Chip
