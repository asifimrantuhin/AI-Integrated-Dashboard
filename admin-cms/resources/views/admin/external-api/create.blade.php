...
@section('title', 'Configure External API')
    ...
    <div class="col-12">
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center">
                <h4 class="mb-0">Configure External API</h4>
            ...
            <div class="card-body">
                <form action="{{ route('admin.external-apis.store') }}" method="POST" id="apiForm">
                    @csrf
                    <div class="row g-3">
                        <div class="col-md-6">
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="module" id="module" class="form-select" required>
                                ...
                                @foreach($modules as $key => $spec)
                                    <option value="{{ $key }}" {{ old('module') === $key ? 'selected' : '' }}>{{ strtoupper($key) }} - {{ $spec['label'] }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-8">
                            ...
                            <input type="url" name="url" id="url" class="form-control" placeholder="https://api.example.com/data" value="{{ old('url') }}" required>
                        ...
                        <div class="col-md-2">
                            ...
                            <select name="method" id="method" class="form-select" required>
                                @foreach(['GET','POST','PUT','PATCH','DELETE'] as $method)
                                    <option value="{{ $method }}" {{ old('method', 'GET') === $method ? 'selected' : '' }}>{{ $method }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-2">
                            ...
                            <select name="data_type" id="data_type" class="form-select" required>
                                ...
                                <option value="xml" {{ old('data_type') === 'xml' ? 'selected' : '' }}>XML</option>
                            </select>
                        ...
                        <div class="col-md-4">
                            ...
                            <div class="input-group">
                                ...
                                <input type="text" name="auth_token" id="auth_token" class="form-control" placeholder="token / key" value="{{ old('auth_token') }}">
                            </div>
                            <small class="form-text text-muted">Token kept securely in database.</small>
                        ...
                        <div class="col-md-2" id="authHeaderWrapper" style="display: none;">
                            ...
                            <input type="text" name="auth_header" id="auth_header" class="form-control" placeholder="X-API-Key" value="{{ old('auth_header') }}">
                        ...
                        <div class="col-md-2">
                            ...
                            <select name="sync_interval" id="sync_interval" class="form-select">
                                @foreach([15, 30, 60, 120, 240, 1440] as $interval)
                                    <option value="{{ $interval }}" {{ old('sync_interval', 60) == $interval ? 'selected' : '' }}>Every {{ $interval }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-12">
                            ...
                            <textarea name="headers" id="headers" class="form-control font-monospace" rows="3" placeholder='{"Content-Type": "application/json"}'>{{ old('headers') }}</textarea>
                        ...
                        <div class="col-md-6">
                            ...
                            <textarea name="query_params" id="query_params" class="form-control font-monospace" rows="3" placeholder='{"from": "{{month_start}}", "to": "{{today}}"}'>{{ old('query_params') }}</textarea>
                            @error('query_params')<div class="text-danger">{{ $message }}</div>@enderror
                        </div>
                        <div class="col-md-6">
                            <label for="body" class="form-label">Request Body (JSON)</label>
                            <textarea name="body" id="body" class="form-control font-monospace" rows="3" placeholder='{"filters": {"company_id": "{{company_id}}"}}'>{{ old('body') }}</textarea>
                            <small class="form-text text-muted">Used for POST/PUT/PATCH requests.</small>
                            @error('body')<div class="text-danger">{{ $message }}</div>@enderror
                        </div>
                        <div class="col-md-6">
                            <label for="items_path" class="form-label">Items Path (dot notation)</label>
                            <input type="text" name="items_path" id="items_path" class="form-control" placeholder="data.records" value="{{ old('items_path') }}">
                            <small class="form-text text-muted">Leave blank if response root is the array.</small>
                        ...
                        <div class="col-12">
                            ...
                            <textarea name="data_mapping" id="data_mapping" class="form-control font-monospace" rows="10" required>{{ old('data_mapping') }}</textarea>
                            <small class="form-text text-muted">Define target table, fields, and upsert keys. The format auto-populates when you pick a module.</small>
...
const hasOldMethod = Boolean(@json(old('method')));
const hasOldItemsPath = Boolean(@json(old('items_path')));
const hasOldQueryParams = Boolean(@json(old('query_params')));
const hasOldBody = Boolean(@json(old('body')));
const hasOldMapping = Boolean(@json(old('data_mapping')));

function populateModuleDefaults(moduleKey) {
    ...
    if (defaults.method && !hasOldMethod) {
        methodField.value = defaults.method;
    }

    if (defaults.items_path && !hasOldItemsPath && !itemsPathField.value) {
        itemsPathField.value = defaults.items_path;
    }

    if (defaults.query_params && !hasOldQueryParams && !queryParamsField.value) {
        queryParamsField.value = JSON.stringify(defaults.query_params, null, 2);
    }

    if (defaults.data_mapping && !hasOldMapping && !mappingField.value) {
        mappingField.value = JSON.stringify(defaults.data_mapping, null, 2);
    }

    if (defaults.body && !hasOldBody && !bodyField.value) {
        bodyField.value = JSON.stringify(defaults.body, null, 2);
    }
}

moduleSelect?.addEventListener('change', function () {
    populateModuleDefaults(this.value);
});
...
document.addEventListener('DOMContentLoaded', function () {
    if (moduleSelect.value) {
        populateModuleDefaults(moduleSelect.value);
    }
