...
class SalesUploadController extends Controller
{
    ...
    {
        $uploads = FileUpload::where('module', 'sales')
            ->orderBy('created_at', 'desc')
            ->paginate(20);

        return view('admin.upload.sales.index', compact('uploads'));
    }

    public function create()
    {
        return view('admin.upload.sales.create');
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $path = $file->store('uploads/sales/' . $dataType);
            ...
            Excel::import(new SalesDataImport($upload, $dataType), $file);

            $upload->update(['status' => 'completed']);

            return redirect()->route('admin.upload.sales.index')
                ->with('success', 'Sales data uploaded and processed successfully!');
        } catch (\Exception $e) {
            Log::error('Sales upload error: ' . $e->getMessage());
    ...
    public function show($id)
    {
        ...
        return view('admin.upload.sales.show', compact('upload', 'data'));
    }

    public function downloadSample($dataType)
    {
        $samplePath = storage_path('app/samples/sales/' . $dataType . '_sample.xlsx');
